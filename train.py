...
def train_one_epoch(sess, tr_model):
  ...
  while True:
    try:
      _, loss, current_batchsize, log_bias = sess.run(
          # [tr_model.train_op, tr_model.loss, tf.shape(tr_model.lengths)[0]])
          [tr_model.train_op, tr_model.loss, tr_model.batch_size, tr_model.log_bias])
      tr_loss += loss
      if (i+1) % PARAM.minibatch_size == 0:
        ...
        print("MINIBATCH %05d: AVG.LOSS %04.6f, "
...
def train():
  ...
  with g.as_default():
    ...
    if PARAM.resume_training.lower() == 'true':
      ...
      with open(os.path.join(PARAM.SAVE_DIR, 'train.log'), 'a+') as f:
    ...
    for epoch in range(PARAM.start_epoch, PARAM.max_epochs):
      ...
      ckpt_name = "nnet_iter%d_lrate%e_trloss%.4f_cvloss%.4f_duration%ds" % (
          epoch + 1, PARAM.learning_rate, tr_loss, val_loss, end_time - start_time)
      ...
      if val_loss < loss_prev:
        ...
        msg = ("Iteration %03d: TRAIN AVG.LOSS %.4f, lrate%e, VAL AVG.LOSS %.4f,\n"
               "%s, ckpt(%s) saved,\nEPOCH DURATION: %.2fs") % (
            epoch + 1, tr_loss, PARAM.learning_rate, val_loss,
      ...
      if (rel_impr < PARAM.start_halving_impr) or (reject_num >= 2):
        reject_num = 0
        PARAM.learning_rate *= PARAM.halving_factor
        tr_model.assign_lr(sess, PARAM.learning_rate)
